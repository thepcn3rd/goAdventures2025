...
type Configuration struct {
	...
	NoteColors           string          `json:"_noteColors"`             // Colors for different types of notes in the graph
	NetworkNodeColors    NodeColorStruct `json:"networkNodeColors"`       // Colors for different types of nodes in the graph
}

type NodeColorStruct struct {
	Internet string `json:"internet"` // Color for internet nodes
	External string `json:"external"` // Color for external nodes
	Internal string `json:"internal"` // Color for internal nodes
}
...
func (c *Configuration) CreateConfig(f string) error {
	...
	c.NoteColors = "Set the protocol on an import to 'nat' and the color will be set to the specified below and the direction in the node will be set to 'NAT' in the graph"
	c.NetworkNodeColors = NodeColorStruct{
		Internet: "Magenta",
		External: "Red",
		Internal: "Blue",
	}
...
func CalcNodeType(ipNode IPNode, config Configuration) IPNode {
	...
	for _, externalNetwork := range config.ExternalNetworks {
		if IPInSubnet(ipNode.IPAddress, externalNetwork) {
			...
			ipNode.NodeColor = config.NetworkNodeColors.External // Set the color for external nodes
		}
	}
	...
	for _, internalNetwork := range config.InternalNetworks {
		if IPInSubnet(ipNode.IPAddress, internalNetwork) {
			...
			ipNode.NodeColor = config.NetworkNodeColors.Internal // Set the color for internal nodes
		}
	}
	...
	if ipNode.NodeType != "External" && ipNode.NodeType != "Internal" && ipNode.NodeType != "Ignored" {
		...
		ipNode.NodeColor = config.NetworkNodeColors.Internet // Set the color for internet nodes
	}
...
func main() {
	...
	CSVPtr := flag.String("csv", "", "CSV file to load into the database (not implemented yet)")
	...
	if len(csvFile.Rows) == 0 || *CSVPtr == "" {
		...
		fmt.Println("source_ip,destination_ip,protocol,destination_port")
		fmt.Println("Example row:")
		fmt.Println("10.1.1.1,10.2.2.2,tcp,80")
	} else {
