...
[DNS Server](/dnsServer/main.go) - Simple DNS Server written in Golang to perform local lookups and then forward any additional requests to an upstream DNS server.  Designed it to be simple and to create a log or send the log by syslog to Elastic inside of a Security Onion Solution.

[SSL Reverse Proxy](/sslReverseProxy/main.go) - Setup a guacamole server by using docker to connect to multiple VM instances installed on a proxmox.  By default the guacamole server does not enable SSL, instead of modifying the docker image or setting up a nginx reverse proxy; I built this custom reverse proxy from a previous project.  This also allows logging to a local file or by syslog to Elastic.
