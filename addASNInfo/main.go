...
func findASN(ip string) {
	...
	for _, asn := range asn.ASNInfo {
		...
		if matchStart && matchEnd {
			...
			outputString := []string{ip, asn.ASN, removeBadChars(asn.ASNName), removeBadChars(asn.ASDomain), asn.Country, removeBadChars(asn.CountryName), asn.ContinentName}
