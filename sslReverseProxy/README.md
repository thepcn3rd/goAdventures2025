...
Guacamole, an open-source remote desktop gateway, does not natively support TLS when deployed via Docker. By default, Guacamole listens on HTTP, which poses a security risk, especially when authentication occurs over plain text. Apache recommends placing a reverse proxy (e.g., nginx) in front of Guacamole to secure connections.

This project addresses two primary use cases:

1. **Home Lab Management**: Managing multiple VMs on Proxmox requires a secure and seamless remote desktop experience. Tools like noVNC or Remmina have limitations, particularly with copy-paste functionality between hosts and VMs.
...
The custom Go-based TLS reverse proxy provides a lightweight, customizable solution to secure Guacamole connections without modifying its Docker image or relying on nginx.
...
2. **Refinement**: Hosting the proxy directly on the VM running Guacamole allowed direct communication with Guacamole's private Docker network (e.g., `172.17.0.4`), eliminating the need to expose port `8080` on the VMs interface.
